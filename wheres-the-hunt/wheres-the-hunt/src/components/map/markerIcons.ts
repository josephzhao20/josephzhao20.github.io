  ...
  return `
    <svg width="34" height="44" viewBox="0 0 34 44" xmlns="http://www.w3.org/2000/svg">
      <path d="M17 1 C7.6 1 1 8 1 16.5 C1 27 17 43 17 43 C17 43 33 27 33 16.5 C33 8 26.4 1 17 1 Z"
            fill="${fill}" stroke="#211D17" stroke-width="2.5"/>
      <circle cx="17" cy="16.5" r="9.5" fill="#F6F1E7" stroke="#211D17" stroke-width="1.5"/>
      <path d="M17 9 L20 16 L17 24 L14 16 Z" fill="${featured ? '#C9602E' : '#211D17'}"/>
    </svg>
  `;
}

export function createAdventureIcon(opts: { featured?: boolean } = {}) {
  const fill = opts.featured ? '#C9602E' : '#2E3D33';
  return L.divIcon({
